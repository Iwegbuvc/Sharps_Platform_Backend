...
const createProduct = async (req, res) => {
  try {
    ...
    if (!['interiors', 'clothing', 'accessories', 'shirts', 'shoes', 'artifacts', 'others'].includes(normalizedCategory)) {
...
const getProducts = async (req, res) => {
  try {
    ...
    if (normalizedSubcategory) filter.subcategory = normalizedSubcategory;
    if (gender) filter.gender = gender;
    if (isNew) filter.isNew = isNew === "true";
    if (featured) filter.featured = featured === "true";
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (
      category !== undefined &&
      normalizedCategory &&
      !["interiors", "clothing"].includes(normalizedCategory)
    ) {
      return res.status(400).json({
        message: `Invalid category: ${category}`,
      });
    ...
    const updateData = {
      ...
      price: price ?? existingProduct.price,
      stock: stock ?? existingProduct.stock,
      category: category !== undefined ? normalizedCategory : existingProduct.category,
      subcategory:
        subcategory !== undefined ? normalizedSubcategory : existingProduct.subcategory,
      gender: gender ?? existingProduct.gender,
      isNew: isNew ?? existingProduct.isNew,
      featured: featured ?? existingProduct.featured,
    };
